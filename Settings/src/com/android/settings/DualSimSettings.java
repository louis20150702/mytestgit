...
import com.android.internal.telephony.TelephonyIntents;
 //revo lyq 20141108 for dualsim data settings   
import android.net.ConnectivityManager;
...
public class DualSimSettings extends PreferenceActivity implements
   ...
   private ConnectivityManager mConnManager;	
	...
	if(FeatureOption.PRJ_FEATURE_MULTI_PRJ_TRX_VID_Z40NOVA){
	        mRoot.removePreference(findPreference("sim_reply_setting"));
	}
    ...
    private void updateDataSummary() {
        ...
        Log.d(TAG, "updateDataSummary 222");   

	 //revo lyq 20141108 for dualsim data settings   
        //CharSequence[] summaries = getResources().getTextArray(R.array.dualsim_summaries);
        CharSequence[] summaries = getResources().getTextArray(R.array.dualsim_setting_entries_extr);
		...
		//diwei add for dualsim data settings   
		 if(!isEnabled)   mData.setEnabled(false);
        ...
        if (activeSimCount == mDefaultValue) {
            ...
            if(activeSimCount >= mSeleteActiveSimMinCount){
               ...
               //revo lyq 20141108 for dualsim data settings   
	          String[] default_entries_array = getResources().getStringArray(
	                R.array.dualsim_setting_entries_extr);
			  
	          String[] default_value_array = getResources().getStringArray(
	                R.array.dualsim_setting_values);
                String[] entries_array =new String[2];
		  String[] value_array  =new String[2];

		  if(defaultPhoneId==0){
                   entries_array[0]=default_entries_array[0];
		     value_array[0]=default_value_array[0];	   
		  }else{
                   entries_array[0]=default_entries_array[1];
		     value_array[0]=default_value_array[1];	   
		  }
		  
	         entries_array[1]=default_entries_array[2];
		  value_array[1]=default_value_array[2];	   		  
		  mData.setEntries(entries_array);
		  mData.setEntryValues(value_array);
		  //revo lyq 20141108 for dualsim data settings   
           ...
           //revo lyq 20141108 for dualsim data settings   
            if(true){
		int phone_id=TelephonyManager.getDefaultDataPhoneId(this);
	       boolean data_on=mConnManager.getMobileDataEnabledByPhoneId(phone_id);	
		if(!data_on){
	            mData.setValue("-1");
	            mData.setSummary(summaries[2]);
		}else{
  ...
  //revo lyq 20141108 for dualsim data settings   
    private void doAutoDataConnection (int phoneId,boolean on) {
        mConnManager.setMobileDataEnabledByPhoneId(phoneId, on);
       // moblieDataOpeningFlag = true;
        if (phoneId != -1) {
            Intent intent = new Intent();
            intent.putExtra("SIM_ID", phoneId);
            intent.putExtra("SETTING_ID", phoneId);
            intent.setAction(Intent.ACTION_DATA_CONNECTION_CHANGED);
            sendBroadcast(intent);
        }
    }

  private void setDataSummaryByUser(int num) {
...
//revo lyq 20141108 for dualsim data settings   
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        ...
        if (KEY_DATA.equals(key)) {
	     ...
	     if(true){	
	            if(phoneId==-1){
			  int t_phoneId=TelephonyManager.getDefaultDataPhoneId(this);
			  setDataSummaryByUser(2);
			  doAutoDataConnection(t_phoneId,false);
	            }else if (phoneId == TelephonyManager.getDefaultDataPhoneId(this)){
			int phone_id=TelephonyManager.getDefaultDataPhoneId(this);
		       boolean data_on=mConnManager.getMobileDataEnabledByPhoneId(phone_id);	
			if(!data_on&&mData.getValue().equals("-1")){
			  doAutoDataConnection(phone_id,true);	
			  setDataSummaryByUser(phone_id);	
			}
		
	            }else if (phoneId != TelephonyManager.getDefaultDataPhoneId(this)){
	                startUpdateDataSettings(phoneId);
	               //revo lyq 20141108	
	                doAutoDataConnection(phoneId,true); 
	            }
	     }else{
