...
public class TextToSpeechSettings extends SettingsPreferenceFragment implements
    ...
    public void onInitEngine(int status) {
        if (status == TextToSpeech.SUCCESS) {
            ...
            updateWidgetState(false);
            /* SPRD:Modify Bug #277046 in tts, default language status is checking. @{ */
    ...
    private void getSampleText() {
        ...
        /* SPRD:Modify Bug 208233,click the button can't play voice, and exit the page the voice can't stop. @{ */
        String language = defaultLocale.getLanguage();
        if(language == null || language.isEmpty()){
            Locale currentLocale = new Locale("eng", "USA", "");
