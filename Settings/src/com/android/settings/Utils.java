...
public class Utils {
    ...
    public static String getIpv6Addresses(Context context) {
        ...
        try {
            ...
            while (!TextUtils.isEmpty(line = inFile.readLine())) {
                Log.e("Utils", "getIpv6Addresses line: " + line);
                strs = line.split(" ");
                Log.e("Utils", "getIpv6Addresses prop: " + prop);
                if (prop == null) {
                    break;
                }
                if (!strs[5].equals(prop.getInterfaceName())) {
                    continue;
                }
                stringArr = strs[0].toCharArray();
                for (i = 0; i < 32; i++) {
                    addrstr.append(stringArr[i]);
                    if (i % 4 == 3 && i != 31) {
                        addrstr.append(':');
                    }
                }
                addrstr.append('\0');
                if (addrstr.toString().startsWith("fe80:")) {
                    continue;
                }
                Log.e("Utils", "getIpv6Addresses addrstr: " + addrstr.toString());
                fr.close();
                inFile.close();
            ...
            Log.e("Utils", "getIpv6Addresses failed e =  " + e);
        }
        addrstr.delete( 0, addrstr.length());
        return addrstr.toString();
    }
}
