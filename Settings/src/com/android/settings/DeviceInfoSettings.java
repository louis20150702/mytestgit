...
public class DeviceInfoSettings extends RestrictedSettingsFragment {
    ...
    public void onCreate(Bundle icicle) {
        ...
        //revo lyq 20140916
        removePreference("RecoverySystemUpdate");   
        removePreference(KEY_SYSTEM_UPDATE_SETTINGS); 

        setStringSummary(KEY_FIRMWARE_VERSION, Build.VERSION.RELEASE);
        findPreference(KEY_FIRMWARE_VERSION).setEnabled(true);
		if(FeatureOption.PRJ_FEATURE_FIXED_BASEBAND_VERSION){			
			StringBuffer sbuffer = new StringBuffer();
			sbuffer.append("TM_BASE_MP_W14.46|sc7731g_CP0_modem|11-21-2014 19:13:59");
			sbuffer.append("\n");
			sbuffer.append("MOCORTM_14B_TSHARK28_W14.47_Debug|CP2_WCN_14E_W14.47_Release|sc7731g_modem|11-21-2014 19:15:50");
			findPreference(KEY_BASEBAND_VERSION).setSummary(sbuffer.toString());
		}else{
	        /* SPRD: add CP2 version for bug 268191 @{ */
	        if (SystemProperties.get("ro.macro.baseband_version").equals("")) {		
		        setValueSummary(KEY_BASEBAND_VERSION, "gsm.version.baseband");
	        }else{
		    		findPreference(KEY_BASEBAND_VERSION).setSummary(SystemProperties.get("ro.macro.baseband_version"));
			}
	        /* @} */
		}
       //lyx phone model
       //setStringSummary(KEY_DEVICE_MODEL, SystemProperties.get("ro.product.model") + getMsvSuffix());
       setStringSummary(KEY_DEVICE_MODEL, SystemProperties.get("ro.macro.phone_model"));
       if(SystemProperties.get("ro.macro.phone_model").equals("")){
          setStringSummary(KEY_DEVICE_MODEL, SystemProperties.get("ro.product.model"));
       }
        setValueSummary(KEY_EQUIPMENT_ID, PROPERTY_EQUIPMENT_ID);
        setStringSummary(KEY_BUILD_NUMBER, Build.DISPLAY);
        findPreference(KEY_BUILD_NUMBER).setEnabled(true);
	 if(SystemProperties.get("ro.macro.kernel_version").equals("")){
	        findPreference(KEY_KERNEL_VERSION).setSummary(getFormattedKernelVersion());
	 }else{
	        findPreference(KEY_KERNEL_VERSION).setSummary(SystemProperties.get("ro.macro.kernel_version"));
	 }
	 //lgz revo csl update
    if(FeatureOption.PRJ_FEATURE_MULTI_PRJ_CUSTOMER_MALAIA45S)
         	{
         	setStringSummary(KEY_BUILD_NUMBER, "V2_MY_ninetology_V01");
         	}else{
            setStringSummary(KEY_BUILD_NUMBER, Build.DISPLAY);
         	}
		...
		//lyx remove Hardware version
	if(FeatureOption.PRJ_FEATURE_MULTI_PRJ_TRX_VID_Z40NOVA
			|| FeatureOption.PRJ_FEATURE_MULTI_PRJ_CUSTOMER_MALAIA45S 
			|| FeatureOption.PRJ_FEATURE_MULTI_PRJ_CUSTOMER_WOOMexico 			
			|| FeatureOption.PRJ_FEATURE_MULTI_PRJ_CUSTOMER_GFIVE_BASE
			|| FeatureOption.PRJ_FEATURE_MULTI_PRJ_CUSTOMER_FROG){
