...
public class WifiApEnabler {
    ...
    //To save the wifi open state when open softap,
    //it will clear when exiting from the current Softap Setting UI
    //For Bug#310883
    private boolean mWifiOpened = false;
    ...
    public void setSoftapEnabled(boolean enable) {
        ...
        if (!supportBtWifiSoftApCoexit) {
            ...
            if (enable
                    && ((btState == BluetoothAdapter.STATE_ON) || (btState == BluetoothAdapter.STATE_TURNING_ON))) {
        ...
        /* SPRD: Modify Bug 316222 show tip for wifi hotstop by mobile data disabled @{ */
        if (!mConnMgr.getMobileDataEnabled() && enable) {
    ...
    public void updateConfigSummary(WifiConfiguration wifiConfig) {
        String s = mContext.getString(
             ...
             //lyx phone space model
            s=android.os.SystemProperties.get("ro.macro.wifiap_model"); 
            if(s.equals("")){
              s=android.os.SystemProperties.get("ro.product.model"); //revo lyq 20140418  for wifi_tether_configure_ssid_default
            }            
