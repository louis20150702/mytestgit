...
public class AdvancedWifiSettings extends SettingsPreferenceFragment
    ...
    private void initPreferences() {
        //add by spreadst_lc for cmcc wifi feature start
        CheckBoxPreference mAutoConnect = (CheckBoxPreference)findPreference(KEY_AUTO_CONNECT);
        if (supportCMCC) {
            boolean mAutoConnectChecked = WifiConnectionPolicy.getManualConnectPolicy(getActivity())== WifiConnectionPolicy.MANUAL_CONNECT_DISABLED;
            mAutoConnect.setChecked(mAutoConnectChecked);
            mAutoConnect.setEnabled(mWifiManager.isWifiEnabled());
        } else {
            if (mAutoConnect != null) getPreferenceScreen().removePreference(mAutoConnect);
        }
        //add by spreadst_lc for cmcc wifi feature end
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen screen, Preference preference) {
        ...
        //            ((CheckBoxPreference) preference).isChecked() ? 1 : 0);
            ...
            WifiConnectionPolicy.setManualConnectPolicy(getActivity(), ((CheckBoxPreference)preference).isChecked() ? WifiConnectionPolicy.MANUAL_CONNECT_DISABLED : WifiConnectionPolicy.MANUAL_CONNECT_ENABLED);
        } else {
    ...
    private void initCellularWLANPreference() {
        ListPreference pref = (ListPreference) findPreference(KEY_MOBILE_TO_WLAN_POLICY);
        pref.setEnabled(mWifiManager.isWifiEnabled());
