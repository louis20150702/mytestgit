...
/*revo lyq 20141022 for remove touchlight */
...
public class DisplaySettings extends SettingsPreferenceFragment implements
    ...
    private Preference mLedsSettings;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        /* HWSettings {@*/
        Boolean extra = getActivity().getIntent().getBooleanExtra("onlyforfontsettings", false);
        ...
        if (mScreenSaverPreference != null
         ...
         //revo lyq 20141028  for lights		
        mLedsSettings = findPreference("LedsSettings");	 	
        if (!FeatureOption.PRJ_FEATURE_LEDS_SETTINGS) {
            getPreferenceScreen().removePreference(mLedsSettings);
        }
        ...
        initTimeoutPreferenceValue((int)currentTimeout);
        /* @} */
        ...
        /* SPRD:Modify Bug 318961, remove touch light @{ */
        /*revo lyq 20141022 for remove touchlight->!fileIsExists() */
        if (FeatureOption.PRJ_FEATURE_REMOVE_TOUCH_LIGHT_MENU) {
        ...
        if(HWSettings.HW_SETTINGS && extra){
            PreferenceScreen prefScreen = getPreferenceScreen();
            prefScreen.removePreference(mAccelerometer);
            prefScreen.removePreference(mScreenTimeoutPreference);
            prefScreen.removePreference(mTouchLightTimeoutPreference);
            prefScreen.removePreference(mNotificationPulse);            
            prefScreen.removePreference(findPreference("wallpaper"));
            prefScreen.removePreference(findPreference("brightness"));
			if(getResources().getBoolean(
                        com.android.internal.R.bool.config_dreamsSupported) == true){
            	prefScreen.removePreference(findPreference("screensaver"));
			}
        }
    }
       ...
       if(currentTimeout ==-2){
            ...
            final CharSequence[] values = preference.getEntryValues();
                ...
                if(currentTimeout != Long.parseLong(values[values.length-1].toString())){
                    mScreenTimeoutPreference.setValue(values[values.length-1].toString());
                }
                /* @} */
		  ...
		  //revo lyq 20141215 for values.length
     ...
     */
    private void initTimeoutPreferenceValue(int currentTimeout) {
