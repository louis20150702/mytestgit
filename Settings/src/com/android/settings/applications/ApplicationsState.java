...
public class ApplicationsState {
    ...
    public static class AppEntry extends SizeInfo {
        ...
        void setFirstInstallTime(Context context) {
            ...
            try {
                PackageInfo pak = pm.getPackageInfo(info.packageName, 0);
                installTime = pak.firstInstallTime;
            } catch (NameNotFoundException e) {
                Log.d(TAG, "sorry, can not find this package : "+info.packageName);
    ...
    public static final AppFilter ON_SD_CARD_FILTER = new AppFilter() {
        ...
        @Override
        public boolean filterApp(ApplicationInfo info) {
            // SPRD: ADD Just for filter sdcard app.
            return mCanBeOnSdCardChecker.checkAppForSdcardView(info);
        }
    };
    ...
    public class Session {
        ...
        void handleRebuildList() {
			...
			/* remove calend_sync menu in app manage@lihong 20150330 */
			if (FeatureOption.PRJ_FEATURE_PRJ_DIKA_P4511) {
				if ("com.sprd.caldav_sync".equals(info.packageName)) {
					continue;
				}
			}
    ...
    /* SPRD: add beatch uninstall function for bug 283586 @{ */
    public static final Comparator<AppEntry> TIME_COMPARATOR = new Comparator<AppEntry>() {
        private final Collator sCollator = Collator.getInstance();

        @Override
        public int compare(AppEntry object1, AppEntry object2) {
            if (object1.installTime < object2.installTime)
                return 1;
            if (object1.installTime > object2.installTime)
                return -1;
            return sCollator.compare(object1.label, object2.label);
