...
import com.android.settings.HWSettings;
import com.android.settings.R;
import com.android.settings.Settings.ManageApplicationsActivity;
import com.android.settings.Settings.SprdManageApplicationsActivity;
import com.android.settings.Settings.SprdManageTasksActivity;
...
/* hw settings {@*/
import com.android.settings.SettingsTabContainer;
...
final class CanBeOnSdCardChecker {
    ...
    /**
     * check if the app on external sdcard.
     */
    boolean checkAppForSdcardView(ApplicationInfo info) {
        boolean canBe = false;
    ...
    boolean check(ApplicationInfo info) {
        ...
        if ((info.flags & ApplicationInfo.FLAG_EXTERNAL_STORAGE) != 0) {
            ...
            /* SPRD: MODIFY let the preloadapp can not move to sdcard @{ */
...
public class ManageApplications extends Fragment implements
    ...
    // SPRD：ADD for cmcc authentication
    public static final int KILL_APPS_SERVICES = MENU_OPTIONS_BASE + 9;
    // sort order
    private int mSortOrder = SORT_ORDER_ALPHA;

    private ApplicationsState mApplicationsState;

    // wangxiang add for hw begin
    private boolean HW_MANAGER = HWSettings.HW_SETTINGS;
    SettingsTabContainer mContainer;
    ArrayList<String> mTitles;
    static View mBtnClearCache;
    Context mCacheContext;
    long mSumCacheSizes = 0l;
    static Context mTabContext ;
	...
	/*Handler cacheHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			TextView localTextView = (TextView)mBtnClearCache.findViewById(R.id.cache_amount_textview);
			Bundle bundle = msg.getData();
			String text = bundle.getString("btnText");
			localTextView.setText(text);
		}
	};*/

	public /*static*/ void updateTotalCache() {
		Log.i("wangxiang", "updateTotalCache");
		if(mBtnClearCache != null) {
			TextView localTextView = (TextView)mBtnClearCache.findViewById(R.id.cache_amount_textview);
			mSumCacheSizes = mApplicationsState.sumCacheSizes();
			Log.i("wangxiang", "updateTotalCache 222 mSumCacheSizes = " + mSumCacheSizes);
			String cache = getActivity().getString(R.string.chache_amount)
					+ mSumCacheSizes/1024 + "KB";
			localTextView.setText(cache);
		} else {
			Log.i("wangxiang", "updateTotalCache  mBtnClearCache == null");
		}
	}

	
    // wangxiang add for hw end
    public static class TabInfo implements OnItemClickListener {
	    ...
	    ApplicationsState mApps;//wangxiang for hw
	    ...
	    if(HWSettings.HW_SETTINGS) { //wangxiang for hw
	    	mApps = apps;
	    }
            mLabel = label;
            mListType = listType;
	    ...
	    if(HWSettings.HW_SETTINGS) { //wangxiang for hw
			mRootView = inflater.inflate(mListType == LIST_TYPE_RUNNING
					...
					: R.layout.manage_applications_apps_hw, null);
	    } else {
	            mRootView = inflater.inflate(mListType == LIST_TYPE_RUNNING
	                    ? R.layout.manage_applications_running
		...
		if(HWSettings.HW_SETTINGS) { //wangxiang for hw
			mBtnClearCache = (View)mListContainer.findViewById(R.id.clear_cache_btn);
			Log.i("wangxiang", "build");
			if(mBtnClearCache != null && mFilter == FILTER_APPS_ALL) {
				mBtnClearCache.setVisibility(View.VISIBLE);				
				//TextView localTextView = (TextView)mBtnClearCache.findViewById(R.id.cache_amount_textview);
				//String cache = mTabContext.getString(R.string.chache_amount)
				//	+ mApps.sumCacheSizes()/1024 + "KB";
				//localTextView.setText(cache);
				//Log.i("wangxiang", "cache = " + cache);
			}
		}
            ...
            if (mFilter == FILTER_APPS_SDCARD) {
                if (mContainerService != null) {
                    try {
                        final long[] stats = mContainerService.getFileSystemStats(
                                ...
                                Environment.getExternalStoragePath().toString());
                        mTotalStorage = stats[0];
                        mFreeStorage = stats[1];
                    } catch (RemoteException e) {
                        Log.w(TAG, "Problem in container service", e);
                    } catch (Exception ex) {
                        Log.w(TAG, "externalStorage status exception", ex);
                    }
                }
                ...
                if (mApplications != null) {
                    ...
                    for (int i=0; i<N; i++) {
                        ...
                        /*revo lgz Merge partition bug*/ 
                        if (emulatedStorage&&!FeatureOption.PRJ_FEATURE_PRJ_STORAGE_MERGE_PARTITION) {
                            mAppStorage += ae.externalCodeSize + ae.externalDataSize;
                        }
                    }
                }
    ...
    class MyPagerAdapter extends PagerAdapter
        ...
        public void onPageSelected(int position) {
            mCurPos = position;
            if(HW_MANAGER) { //wangxiang add for hw
            	ManageApplications.this.mContainer.selectTab(position);
            }
        }
    ...
    static class ApplicationsAdapter extends BaseAdapter implements Filterable,
        ...
        public void rebuild(boolean eraseold) {
            ...
            /*revo lgz Merge partition bug*/ 
            if (emulated && !FeatureOption.PRJ_FEATURE_PRJ_STORAGE_MERGE_PARTITION) {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        /* SPRD: Modify 20140118 Spread of bug269951, TaskManager and ApplicationManager
         * performance optimization
         */
        if (className.equals(SprdManageApplicationsActivity.class.getName())) {
            defaultListType = LIST_TYPE_DOWNLOADED;
        } else if (className.equals(SprdManageTasksActivity.class.getName())) {
            defaultListType = LIST_TYPE_RUNNING;
        } else if (className.equals(RunningServicesActivity.class.getName())
        ...
        mTabs.add(tab);
        if(HW_MANAGER) {//wangxiang for hw
		mCacheContext = getActivity();
		mTabContext = getActivity();
        	mTitles = new ArrayList<String>();
        	mTitles.add(getActivity().getString(R.string.filter_apps_third_party));
        }
        // for double T.
        // if external sdcard is mounted, the tab is added.
        // SPRD:ADD when the primary is not internal,add the tab.
        if ((Environment.STORAGE_TYPE_EMMC_INTERNAL != Environment.getStorageType()
            ||FeatureOption.PRJ_FEATURE_STORAGE_MERGE_PARTITION_DISPLAY_SDCARD)
                && (Environment.getExternalStoragePathState().equals(Environment.MEDIA_MOUNTED))) {
            tab = new TabInfo(this, mApplicationsState, getActivity()
                    .getString(R.string.filter_apps_onsdcard),
                    LIST_TYPE_SDCARD, this, savedInstanceState);
            mTabs.add(tab);
            if(HW_MANAGER) { //wangxiang add for hw
            	mTitles.add(getActivity().getString(R.string.filter_apps_onsdcard));
            }
        }
        ...
        mTabs.add(tab);
        if(HW_MANAGER) { //wangxiang add for hw
        	mTitles.add(getActivity().getString(R.string.filter_apps_running));
        }
        ...
        mTabs.add(tab);
        if(HW_MANAGER) { //wangxiang add for hw
        	mTitles.add(getActivity().getString(R.string.filter_apps_all));
        }
        ...
        mTabs.add(tab);
        if(HW_MANAGER) { //wangxiang add for hw
        	mTitles.add(getActivity().getString(R.string.filter_apps_disabled));
        }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View rootView = null;
        if(HW_MANAGER) { //wangxiang for hw
        	rootView = mInflater.inflate(R.layout.manage_applications_content_hw,
                    container, false);
        } else {
        	rootView = mInflater.inflate(R.layout.manage_applications_content,
                container, false);
        }
		...
		if(!mApplicationsState.haveDisabledApps()) {
			mTitles.remove(mTitles.indexOf(getActivity().getString(R.string.filter_apps_disabled)));
		}
		
        	mContainer = (SettingsTabContainer)rootView.findViewById(R.id.tab_container);
        	mContainer.createTabs(mTitles.size(), mTitles);
        	mContainer.setTabClickListener(new SettingsTabContainer.TabClickListener() {
				
				@Override
				public void onTabClicked(int paramInt) {
					// TODO Auto-generated method stub
					ManageApplications.this.mViewPager.setCurrentItem(paramInt);
					ManageApplications.this.updateCurrentTab(paramInt);
				}
			});
		//ManageApplications.this.mViewPager.setCurrentItem(1);
        } else { 
	...
	if(HW_MANAGER) { //wangxiang for hw  BUG
    ...
    public void onStart() {
        super.onStart();
		if(HW_MANAGER) { //wangxiang add for hw
			if(mBtnClearCache != null && mBtnClearCache.getVisibility() == View.VISIBLE) {
				Log.i("wangxiang", "onStart");
				updateTotalCache();
			}
		}			
    }
    ...
    public void onDestroyView() {
        ...
        for (int i=0; i<mTabs.size(); i++) {
            ...
            // SPRD:ADD release the session.
            if (null != mTabs.get(i).mApplications) {
                mTabs.get(i).mApplications.release();
            }
        }
    }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        menu.add(0, RESET_APP_PREFERENCES, 4, R.string.reset_app_preferences)
                .setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER);
        /* SPRD: Modify 20140508 Spreadst of Bug 309827, OptionsMenu display abnormal @{ */
        /* SPRD：ADD for cmcc authentication @{ */
        menu.add(0, KILL_APPS_SERVICES, 3, R.string.menu_kill_apps_services)
                .setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER);
        /* @} */
        /* @} */
        updateOptionsMenu();
    ...
    void updateOptionsMenu() {
        ...
        if (mCurTab != null && mCurTab.mListType == LIST_TYPE_RUNNING) {
            ...
            // SPRD：ADD for cmcc authentication
            mOptionsMenu.findItem(KILL_APPS_SERVICES).setVisible(!showingBackground);
            ...
            // SPRD：ADD for cmcc authentication
            mOptionsMenu.findItem(KILL_APPS_SERVICES).setVisible(false);
        }
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if ((menuId == SORT_ORDER_ALPHA) || (menuId == SORT_ORDER_SIZE)) {
            ...
            buildResetDialog();
        /* SPRD：ADD for cmcc authentication @{ */
        } else if(menuId == KILL_APPS_SERVICES){
            KillApps rpu = new KillApps(ManageApplications.this.getActivity(),mam,mpm);
            rpu.killService();  // stop service first before forceStopPackage, service can run the function of onDestroy
            rpu.killApp();
        }else {
        /* @} */
    ...
    public void updateCurrentTab(int position) {
        ...
        if(HW_MANAGER) { //wangxiang add for hw
	        //if(mCurTab.mListContainer != null) {
	        	//mBtnClearCache = (View)mCurTab.mListContainer.findViewById(R.id.clear_cache_btn);
	        	if(mBtnClearCache != null && mBtnClearCache.getVisibility() == View.VISIBLE ) {
			Log.i("wangxiang", "updateCurrentTab");
	        		updateTotalCache();
	        		mBtnClearCache.setOnClickListener(new ClearCacheButtonOnClickListener());
	        	}
	        //}
        }
...
//wangxiang add for hw begin
private class ClearCacheButtonOnClickListener implements View.OnClickListener{
	private static final int DELAY_MSG_MILLISEC = 200;
	private static final int DELAY_MSG_TOAST = 3000;
    private static final int MSG_CLEAR_CACHE = 100;
    private static final int MSG_TOAST_RESULT = 101;
	//Handler mHandler;
	//Context mCacheContext = ManageApplications.mContext;
	...
	void clearAppsCache() {		
		int mAppEntriesSize = mApplicationsState.mAppEntries.size();
		mSumCacheSizes = mApplicationsState.sumCacheSizes();
		if(mAppEntriesSize > 0) {
			for (int i = 0; i < mAppEntriesSize; i++) {
				final ApplicationsState.AppEntry appEntry = (ApplicationsState.AppEntry)mApplicationsState.mAppEntries.get(i);
				mCacheContext.getPackageManager().deleteApplicationCacheFiles(appEntry.info.packageName,  new IPackageDataObserver.Stub() {
							public void onRemoveCompleted(final String packageName, final boolean succeeded) {
								mApplicationsState.requestSize(appEntry.info.packageName);
							}
						}
					);
			}
		...
		//Toast.makeText(mCacheContext, mCacheContext.getString(R.string.chache_clearing) 
		//		, Toast.LENGTH_SHORT).show();
		
		//Message msg2 = new Message();
		//msg2.what = MSG_TOAST_RESULT;
		//mHandler.sendMessageDelayed(msg2,DELAY_MSG_TOAST);
	}

	void showToast() {
		long mSumCacheSizeNow = mApplicationsState.sumCacheSizes();		
		long delta = mSumCacheSizes - mSumCacheSizeNow;
		
		Toast.makeText(mCacheContext, mCacheContext.getString(R.string.chache_release_1) + delta/1024 + "KB" + mCacheContext.getString(R.string.chache_release_2)
				, Toast.LENGTH_SHORT).show();
		
		Log.i("wangxiang", "showToast");
		updateTotalCache();
		//delta = 0l;
	}
	...
	mProgressDialog = ProgressDialog.show(mCacheContext, mCacheContext.getString(R.string.clear_cache_btn_text) , mCacheContext.getString(R.string.chache_clearing) , true, false);
	...
	new Thread() {
    	...
    	//mHandler.sendMessageDelayed(msg, DELAY_MSG_MILLISEC);
    }
}
//wangxiang add for hw end
