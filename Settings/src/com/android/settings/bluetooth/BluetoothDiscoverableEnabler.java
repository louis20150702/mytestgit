...
final class BluetoothDiscoverableEnabler implements Preference.OnPreferenceClickListener {
    ...
    private int getDiscoverableTimeout() {
        ...
        if (timeout < 0) {
            String timeoutValue;
	     String	 mStr = android.os.SystemProperties.get("ro.macro.bt_discov_timeout");
		 
		//revo lyq 20140819
        if(mStr.equals("0")){	
		timeoutValue= mSharedPreferences.getString(KEY_DISCOVERABLE_TIMEOUT,
                    VALUE_DISCOVERABLE_TIMEOUT_NEVER);
            }else if(mStr.equals("5m")){
		timeoutValue= mSharedPreferences.getString(KEY_DISCOVERABLE_TIMEOUT,
                    VALUE_DISCOVERABLE_TIMEOUT_FIVE_MINUTES );
	     }else if(mStr.equals("60m")){
		timeoutValue= mSharedPreferences.getString(KEY_DISCOVERABLE_TIMEOUT,
                    VALUE_DISCOVERABLE_TIMEOUT_ONE_HOUR );
	     }else{//2m
		timeoutValue= mSharedPreferences.getString(KEY_DISCOVERABLE_TIMEOUT,
