...
public class ApnEditor extends PreferenceActivity
    ...
    private final static String KEY_APN_TYPE = "apn_type";
    ...
    private static final String IS_RELOADED = "IS_PRELOADED";
    ...
    private int mPhoneId = 0; // SPRD: add by spreadst

    private Uri mUri;
    private Cursor mCursor;
    private boolean mNewApn;
    private boolean mFirstTime;
    private boolean mIsPreloaded = false; // SPRD: for cmcc case, preloaded apns should not be editable or deleted.
    private Resources mRes;
    private TelephonyManager mTelephonyManager;
    private String mRecordApnType;
    // SPRD: modify for bug271734
    private String mApnTypeSummary;
    ...
    private static final String[] sProjection = new String[] {
            ...
            Telephony.Carriers.PRELOAD // SPRD: for cmcc case, preloaded apns should not be editable or deleted.
    };
    ...
    private static final int PRELOAD_INDEX = 22; // SPRD: for cmcc case, preloaded apns should not be editable or deleted.
    ...
    protected void onCreate(Bundle icicle) {
        ...
        mApnType = (Preference) findPreference(KEY_APN_TYPE);
        ...
        getPreferenceScreen().removePreference(mCarrierEnabled); // SPRD: add by spreadst
        ...
        if (TelephonyManager.isMultiSim()) {
            // Read the mPhoneId received from Phone settings.
            mPhoneId = intent.getIntExtra(WirelessSettings.SUB_ID, 0);
            //diwei add for set sim name 
            if(mSimManager !=null){
                this.setTitle(mSimManager.getSimName(mPhoneId,getResources().getString(R.string.apn_edit_ex, mPhoneId + 1)));
            }else{
                this.setTitle(getResources().getString(R.string.apn_edit_ex, mPhoneId + 1));
            }
            Log.d(TAG, "ApnEditor onCreate received mPhoneId: " + mPhoneId);
        } else {
            this.setTitle(R.string.apn_edit);
        }
        /* @} */
        ...
        /* SPRD: modify for bug271734 @{ */
        if(icicle != null){
            mApnTypeSummary = icicle.getString(APN_TYPE,"");
            // SPRD: modify for bug271736
            mIsPreloaded = icicle.getBoolean(IS_RELOADED,false);
        }
        /* @} */
        if (action.equals(Intent.ACTION_EDIT)) {
            ...
            if (mFirstTime || icicle.getInt(SAVED_POS) == 0) {
                ...
                /* SPRD: for multi-sim @{ */
                // mUri =
                // ContentUris.withAppendedId(Telephony.Carriers.CONTENT_URI,
                // icicle.getInt(SAVED_POS));
                mUri = ContentUris.withAppendedId(
                                        Telephony.Carriers.getContentUri(mPhoneId,null), icicle
                                                .getInt(SAVED_POS));
                /* @} */
            }
            mNewApn = true;
        ...
        /* SPRD: for bug 297172: Use common query management Cursor @{ */
    ...
    public void onResume() {
        ...
        /* SPRD: for bug 297172: After hanging up the phone, can not save the new apn normally @{ */
        if(mIsdelete) {
            if(mCursor != null) {
                mCursor.close();
                mCursor = null;
        ...
        if (mIsPreloaded && "cucc".equals(SystemProperties.get("ro.operator", ""))) {
    ...
    private void fillUi() {
        if (mFirstTime) {
            ...
            // SPRD: modify for bug271734
            mApnTypeSummary = checkNull(mCursor.getString(TYPE_INDEX));
            // SPRD: for cmcc case, preloaded apns should not be editable or deleted.
            mIsPreloaded = ("1".equals(mCursor.getString(PRELOAD_INDEX)));
            ...
            if (mNewApn) {
                ...
                if (TelephonyManager.isMultiSim()) {
                    ...
                    // Get the value from appropriate Telephony Property based
                    // on the mPhoneId.
                    numeric = SystemProperties
                                            .get(TelephonyManager.getProperty(TelephonyProperties.PROPERTY_ICC_OPERATOR_NUMERIC, mPhoneId));
        ...
        // SPRD: modify for bug271734
        mApnType.setSummary(mApnTypeSummary);
    ...
    /* SPRD: add new function that user can do multi choice for apn type, and also can edit @{ */
    @Override
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen,
            Preference preference) {
        String key = preference.getKey();
        if (KEY_APN_TYPE.equals(key)) {
            Intent intentApn = new Intent(ApnEditor.this, ApnTypeSettingActivity.class);
            intentApn.setAction("android.intent.action.APNTYPE");
            Bundle bundle = new Bundle();
            mRecordApnType = mApnType.getSummary().toString();
            bundle.putString(KEY_APN_TYPE, mRecordApnType);
            intentApn.putExtras(bundle);
            startActivityForResult(intentApn, 0);
        }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        if (KEY_AUTH_TYPE.equals(key)) {
            ...
            /* SPRD: apn protocol showing problem @{ */
            ...
            /* SPRD: apn roaming protocol showing problem @{ */
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        switch (resultCode) {
            case RESULT_OK:
                ...
                String apnType = b.getString(KEY_APN_TYPE);
                mApnType.setSummary(checkNull(apnType));
                break;

            case RESULT_CANCELED:
                mApnType.setSummary(checkNull(mRecordApnType));
                break;
            default:
                break;
        }
    ...
    protected void onSaveInstanceState(Bundle icicle) {
        ...
        // SPRD: modify for bug271734
        icicle.putString(APN_TYPE, mApnTypeSummary);
        // SPRD: modify for bug271736
        icicle.putBoolean(IS_RELOADED,mIsPreloaded);
    }
    ...
    private boolean validateAndSave(boolean force) {
        ...
        if (mApnType.getSummary() != null) {
            values.put(Telephony.Carriers.TYPE, checkNotSet(mApnType.getSummary().toString()));
        }
        ...
        try {
            mDDS = Settings.System
                    .getInt(getContentResolver(), Settings.System.MULTI_SIM_DATA_CALL);
        } catch (Exception e) {
        ...
        if (mCurMnc != null && mCurMcc != null) {
            ...
            if (mCurMnc.equals(mnc) && mCurMcc.equals(mcc) && mPhoneId == mDDS) {
    ...
    private String getErrorMsg() {
        ...
        String mcc = checkNotSet(mMcc.getText());
        String mnc = checkNotSet(mMnc.getText());
        String apnString = (String) mApnType.getSummary();
        String apnType = checkNotSet(apnString);
        ...
        if (name.length() < 1) {
            ...
            errorMsg = mRes.getString(R.string.error_apn_type_message);
        }
    ...
    protected Dialog onCreateDialog(int id) {
        ...
        if (id == ERROR_DIALOG_ID) {
            ...
            return new AlertDialog.Builder(this)
                    ...
                    /* SPRD: add by spreadst @{ */
                    .setNegativeButton(R.string.apn_edit_quit, new DialogInterface.OnClickListener() {
                        @Override
