...
public class ChooseLockGeneric extends PreferenceActivity {
    ...
    public static class ChooseLockGenericFragment extends SettingsPreferenceFragment {
        ...
        /*liangbo add for silde lock screen item can change 20150206
        PRJ_FEATURE_SLIDE_LOCK_ITEM_SETTING*/
        private static final int SLIDE_LOCK_REQUEST = 102;
        ...
        public void onActivityResult(int requestCode, int resultCode, Intent data) {
            ...
            if (requestCode == CONFIRM_EXISTING_REQUEST && resultCode == Activity.RESULT_OK) {
                ...
                finish();
            } else if(requestCode == SLIDE_LOCK_REQUEST && resultCode == Activity.RESULT_OK){
            	/*liangbo add for silde lock screen item can change 20150206
                PRJ_FEATURE_SLIDE_LOCK_ITEM_SETTING*/
            	mChooseLockSettingsHelper.utils().clearLock(false);
                mChooseLockSettingsHelper.utils().setLockScreenDisabled(false);
                getActivity().setResult(resultCode);
                finish();
            } else {
        ...
        void updateUnlockMethodAndFinish(int quality, boolean disabled) {
            ...
            if (quality >= DevicePolicyManager.PASSWORD_QUALITY_NUMERIC) {
            	...
            	/*liangbo add for silde lock screen item can change 20150206
                PRJ_FEATURE_SLIDE_LOCK_ITEM_SETTING*/
            	if(!disabled && FeatureOption.PRJ_FEATURE_SLIDE_LOCK_ITEM_SETTING){
            		
            		Intent intent = new Intent();
            		ComponentName comp = new ComponentName("com.android.settings",
            						"com.android.settings.slidelock.SlideLockScreenSettingActivity");
            		intent.setComponent(comp);
            		startActivityForResult(intent, SLIDE_LOCK_REQUEST);
            	}else{
            		mChooseLockSettingsHelper.utils().clearLock(false);
