...
public class TetherSettings extends SettingsPreferenceFragment
    ...
    public void onCreate(Bundle icicle) {
        ...
        Preference hotspotSettings = findPreference("hotspot_settings");
        ...
        if (wifiAvailable && !Utils.isMonkeyRunning()) {
            if (SystemProperties.getInt("ro.hotspot.enabled", 0) != 0) {
                getPreferenceScreen().removePreference(mEnableWifiAp);
                getPreferenceScreen().removePreference(wifiApSettings);
            } else {
                getPreferenceScreen().removePreference(hotspotSettings);
    ...
    private class TetherChangeReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context content, Intent intent) {
             ...
             */
                ...
                int newState = intent.getIntExtra(BluetoothPan.EXTRA_STATE, 0);
                int localRole = intent.getIntExtra(BluetoothPan.EXTRA_LOCAL_ROLE, 0);
                if(newState == BluetoothPan.STATE_DISCONNECTED && localRole == BluetoothPan.LOCAL_NAP_ROLE) {
                    updateState();
                }
            }
            /* @} */
            /*
             * SPRD: Add a judge that if action equals
             * ACTION_AIRPLANE_MODE_CHANGED, set whether this preference is
             * enabled by the isAirplaneMode. @{
             */
    ...
    public void onStart() {
        ...
        // SPRD: Add a action that ACTION_AIRPLANE_MODE_CHANGED.
    ...
    private void updateBluetoothState(String[] available, String[] tethered,
        ...
        BluetoothAdapter adapter = BluetoothAdapter.getDefaultAdapter();
	    /* SPRD: if adapter is null ,return directly @{ */
	    if(adapter == null)
	        return;
	    /* @} */
        ...
        if (btState == BluetoothAdapter.STATE_TURNING_OFF) {
            ...
            if (btState == BluetoothAdapter.STATE_ON && bluetoothPan != null &&
                ...
                // mBluetoothTether.setEnabled(true);
                /*
                 * SPRD: Set whether this preference is enabled by the
                 * isAirplaneMode. @{
                 */
                boolean isAirplaneMode = Settings.System.getInt(getContentResolver(),
                        Settings.System.AIRPLANE_MODE_ON, 0) != 0;
          ...
          if (ENABLE_WIFI_AP.equals(preference.getKey())) {
                if (enable) {
                    if (!supportBtWifiSoftApCoexit) {
                        BluetoothAdapter adapter = BluetoothAdapter.getDefaultAdapter();
                        int btState = adapter.getState();
                        if(enable && ((btState == BluetoothAdapter.STATE_ON) || (btState == BluetoothAdapter.STATE_TURNING_ON))) {
